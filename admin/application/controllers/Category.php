...
class Category extends MY_Auth_Controller {
    ...
    public function cat_list() {
        ...
        $row[] = $no;
        $row[] = $rows['cat_name'];
        $row[] = '<img src="'.CUSTOM_BASE_URL.'uploads/category/crop/'.$rows['image'].'" class="img-responsive" height=60 width=80 /></a>';
          
        //add html for action
        $row[] = '<a href="category/edit/'.$rows['cat_id'].'" class="btn  btn-warning" href="#"><i class="fa fa-edit" aria-hidden="true"></i></a>
        <a data-toggle="modal" data-id='.$rows['cat_id'].' data-target="#del-modal" class="btn  btn-danger" href="#"><i class="fa  fa-trash-o" aria-hidden="true"></i></a>';
    ...
    public function create() {
        ...
        else 
        {
            ...
            $value = array('cat_name' => $cat_name,'description' => $description,'image' => $image);
            ...
            $id = $this->input->post('id');
             ...
             else 
             {
                ...
                if($this->image->crop_image_name !="")//if there is image and pdf is null
                {
                    ...
                    $value = array('cat_name' => $cat_name,'description' => $description);
