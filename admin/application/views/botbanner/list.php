          ...
          <section class="content">
            <div class="box">
                <div class="box-header">
                  <a class="btn bg-olive" href="<?= CUSTOM_BASE_URL;?>banner_bottom/create">Create</a>
                </div><!-- /.box-header -->
                <div class="box-body">
...
<!-- /.content -->
    ...
    $(document).ready(function () {//delete_casestudy modal when edit button click
        $('#delModal').on('show.bs.modal', function (e) 
        {
           ...
           // var cropname = $(e.relatedTarget).data('cropname');
            $.ajax({
                type: 'post',
                url: '<?= CUSTOM_BASE_URL . 'banner_bottom/delete' ?>', //Here you will fetch records 
    ...
    $(document).ready(function () {
        ...
        $('#view-modal').on('show.bs.modal', function (e) {
            ...
            $.ajax({
          type: 'post',
          url: '<?= CUSTOM_BASE_URL . 'banner_bottom/view' ?>', //Here you will fetch records 
          data: 'rowid=' + rowid, //Pass $id
          success: function (data) {
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        "ajax": {
        "url": '<?php echo site_url('banner_bottom/bannerlist'); ?>',
        "type": "POST",
        },
        });
    });
...
<?php if ($this->session->flashdata('add')) { ?>
    $("#add_confirm").modal('show');<?php unset($_SESSION['add']); } ?>
<?php if ($this->session->flashdata('update')) { ?>
    $("#update_confirm").modal('show');<?php unset($_SESSION['update']); } ?>
<?php if ($this->session->flashdata('delete')) { ?>
    $("#delete_confirm").modal('show');<?php unset($_SESSION['delete']); } ?>
